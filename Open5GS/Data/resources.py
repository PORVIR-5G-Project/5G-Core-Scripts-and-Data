...
pastas = ['amf', 'ausf', 'bsf', 'nrf', 'nssf', 'pcf', 'smf', 'udm', 'udr', 'upf-1']
...
def get_combined_metrics_data(start_end_timestamps, output_csv="output.csv"):
    ...
    metrics = {
        "mean_cpu_usage": 'sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_irate{cluster="", namespace="cemenin"} * on(namespace,pod) group_left(workload, workload_type)',
        "mean_memory_usage": 'sum(container_memory_working_set_bytes{job="kubelet", metrics_path="/metrics/cadvisor", cluster="", namespace="cemenin", container!="", image!=""} * on(namespace,pod) group_left(workload, workload_type)',
        "mean_receive_bytes": 'sum(rate(container_network_receive_bytes_total{job="kubelet", metrics_path="/metrics/cadvisor", cluster="", namespace="cemenin"}[2m0s]) * on (namespace,pod) group_left(workload,workload_type)',
        "mean_transmit_bytes": 'sum(rate(container_network_transmit_bytes_total{job="kubelet", metrics_path="/metrics/cadvisor", cluster="", namespace="cemenin"}[2m0s]) * on (namespace,pod) group_left(workload,workload_type)'
    }
    ...
    for start, end in start_end_timestamps:
        ...
        for pasta in pastas:
            for metric_col, query_string in metrics.items():
                query_string += f' namespace_workload_pod:kube_pod_owner:relabel{{cluster="", namespace="cemenin", workload="open5gs-{pasta}"}}) by (workload)'
                mean_value = mean_calc(fetch_metrics(query_string, start, end))
                if pasta == 'upf-1':
                    interval_data.at['upf', metric_col] = mean_value
                else:
...
def generate_stacked_barplots_per_component(csv_files, output_image="stacked_barplots.png"):
    ...
    component_colors = {
        ...
        "bsf": (0.5803921568627451, 0.403921568627451, 0.7411764705882353, 1.0),
